...
function breadthFirstDescent(rootId, people, generation) {
  ...
  while (toProcess.length > 0) {
    ...
    if (person) {
      let children = Object.values(people).filter((child) => child.Father == currentId || child.Mother == currentId);
...
function displayPerson(id, people, generation) {
  ...
  if ($(`li[data-id="${person.Id}"]`).length == 0) {
    ...
    const numberOfChildren = Object.values(people).filter((child) => child.Father == id || child.Mother == id).length;
    ...
    const newItem = $(
      `<li data-id='${person.Id}' data-father="${person.Father}" data-mother="${person.Mother}" data-name="${person.Name}" data-birth-year="${birthYear}"  data-death-year="${deathYear}" data-gender='${person.Gender}' class='${theGender}'>${childIndicator} ${listItemContent}<ul data-generation='${generation}' class='${ulState}'></ul></li>`
    );
  ...
  if (!window.gotBios.includes(id)) {
    ...
    if (bioDiv.find("section").length == 0) {
      ...
      bioDiv.find("img").each(function () {
        if ($(this).attr("src").match("https://www.") == null && $(this).closest(".bdDatesLocations").length == 0) {
