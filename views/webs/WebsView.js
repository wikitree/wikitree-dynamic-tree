...
(function () {
    ...
    var ColourArray = ["White","Gold","HotPink","LightCyan","Yellow","AntiqueWhite","MediumSpringGreen","Orange","DeepSkyBlue","PaleGoldenRod","Lime","Moccasin","PowderBlue", "DarkGreen","Maroon", "Navy", "Brown", "Indigo","RoyalBlue", "FireBrick" ,"Blue", "SlateGrey","DarkMagenta","Red", "DarkOrange", "DarkGoldenRod" ,"Green","MediumVioletRed", "SteelBlue" ,"Grey","MediumPurple", "OliveDrab" ,"Purple","DarkSlateBlue", "SaddleBrown" ,"Pink","Khaki", "LemonChiffon" ,"LightCyan","HotPink","Gold","Yellow","AntiqueWhite","MediumSpringGreen","Orange"];
    ...
    WebsView.drawLines = function () {
        ...
        for (let thisGenNum = 0; thisGenNum <  WebsView.numGens2Display  - 1 ; thisGenNum++) {
            ...
            for (let thisPosNum = 0; thisPosNum < numSpotsThisGen; thisPosNum++) {
                ...
                const elementMa = document.getElementById ("lineForPerson" + index + "Ma");
                 if (elementMa) {
                    if ( WebsView.myAhnentafel.list[2*index + 1] && thePeopleList[ WebsView.myAhnentafel.list[2*index + 1] ]) {
                ...
                let Xpa =  0 - numSpotsNextGen * 20 +  ((thisPosNum * 2) / numSpotsNextGen ) * vertSpacing * numSpotsNextGen;
                  Xpa =  0 - dX2 +  thisPosNum * dX3;
                let Ypa = vertSpacing * nextGenNum;

                let Xma = 0 - numSpotsNextGen * 20 +  ((thisPosNum * 2 + 1) / numSpotsNextGen ) * vertSpacing * numSpotsNextGen;
        ...
        for (let index = 2** (WebsView.numGens2Display - 1); index < 2** (WebsView.maxNumGens - 1); index++) {
            const elementPa = document.getElementById ("lineForPerson" + index + "Pa");
            if (elementPa) {elementPa.setAttribute("display", "none");}
            const elementMa = document.getElementById ("lineForPerson" + index + "Ma");
            if (elementMa) {elementMa.setAttribute("display", "none");}
        }
    }
    ...
    function recalcAndDisplayNumGens() {
        if (WebsView.numGens2Display < 1) {
            ...
            if (WebsView.workingMaxNumGens < WebsView.maxNumGens) {
                ...
                showTemporaryMessageBelowButtonBar(WebsView.maxNumGens + " is the maximum number of generations you can display.")
            }
        }
            ...
            // ONLY REDO the WEDGES IFF the maxAngle has changed (360 to 240 to 180 or some combo like that)
             for (let genIndex = WebsView.numGens2Display - 1; genIndex >= 0 ; genIndex--) {
                 for (let index = 0; index < 2**genIndex; index++) {
                    let SVGcode = "";
                    if (genIndex <= 1) {
                        SVGcode = SVGfunctions.getSVGforSector(0, 0,  270*(genIndex + 0.5), (180 - WebsView.maxAngle) / 2 +  90 + 90 +index*WebsView.maxAngle/(2**genIndex),(180 - WebsView.maxAngle) / 2 +  90 + 90 +(index+1)*WebsView.maxAngle/(2**genIndex), 'wedge' + 2**genIndex + 'n' + index, 'black', 2, 'white') ;              

                    } else {
                        SVGcode = SVGfunctions.getSVGforWedge(0, 0,  270*(genIndex + 0.5),  270*(genIndex - 0.5), (180 - WebsView.maxAngle) / 2 +  90 + 90 +index*WebsView.maxAngle/(2**genIndex),(180 - WebsView.maxAngle) / 2 +  90 + 90 +(index+1)*WebsView.maxAngle/(2**genIndex), 'wedge' + 2**genIndex + 'n' + index, 'black', 2, 'white') ;                                  
                    }
    ...
    Tree.prototype.draw = function () {
        ...
        if (this.root) {
            ...
            for (let i = 0; i <= WebsView.numGens2Display; i++) {	
                WebsView.maxDiamPerGen[i] =( 2 ** Math.ceil((WebsView.numGens2Display - i )/2) ) * ( (2 + i%2) * widestBox/3 + theBlobBuffer);
            }
    ...
    Tree.prototype.drawNodes = function (nodes) {
        ...
        nodeEnter
            ...
            .html((ancestorObject) => {
                ...
                if (ancestorObject.ahnNum == 1) {                    
                    return `
                    <div  id=wedgeBoxFor${ancestorObject.ahnNum} class="box" style="background-color: white ; border:1; padding: 0px;">
        ...
        node.attr("transform", function (ancestorObject) {
            ...
            console.log("Transforming:", i, thisGenNum, thisPosNum, numSpotsThisGen, numSpotsMaxGen, "("  ,X, Y, ")", dX);
