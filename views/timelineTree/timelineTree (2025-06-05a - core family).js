...
async function loadTree (event) {
    ...
    for (const famIdx of focusPerson.ParentIn) {
        ...
        for (let i=0; i<ttreeFamilies[famIdx].Children.length; i++) {
            if (ttreeFamilies[famIdx].Children[i].Row < 0) continue;
            ttreePeople[ttreeFamilies[famIdx].Children[i].Row].Visible = true;
        }
    }
...
function updateSibs (event) {
    ...
    for (let i=0; i<ttreePeople.length; i++) {
        ...
        if (sibTypes.includes(person.Type)) {
            ...
            const family = ttreeFamilies[person.ChildIn];
            for (let j = 0; j < family.Parents.length; j++) {
                if ((family.Parents[j].Row < 0) || (ttreePeople[family.Parents[j].Row].Generation < 0)) continue;
...
function updateDisplay (event) {
    ...
    function addToDisplayList(personIdx) {
        ...
        for (let i=0; i<siblings.length; i++) {
            if (ttreePeople[siblings[i].Row].Visible && !(ttreeShow.includes(siblings[i].Row))) {
    ...
    for (const famIdx of ttreePeople[corePersonIdx].ParentIn) {
        for (let i=0; i<ttreeFamilies[famIdx].Children.length; i++) {
            if (ttreeFamilies[famIdx].Children[i].Row < 0) continue;
            if (ttreePeople[ttreeFamilies[famIdx].Children[i].Row].Visible) {
                ttreeShow.splice(spliceLoc,0,ttreeFamilies[famIdx].Children[i].Row);
                spliceLoc += spliceInc;
            }
        }
    }
    for (const famIdx of ttreePeople[corePersonIdx].ParentIn) {
        for (let i=0; i<ttreeFamilies[famIdx].Parents.length; i++) {
            if (ttreeFamilies[famIdx].Parents[i].Row < 0) continue;
            if (ttreeFamilies[famIdx].Parents[i].Row == corePersonIdx) continue;
            if (ttreePeople[ttreeFamilies[famIdx].Parents[i].Row].Visible) {
                ttreeShow.splice(spliceLoc,0,ttreeFamilies[famIdx].Parents[i].Row);
                spliceLoc += spliceInc;
            }
        }
    }
