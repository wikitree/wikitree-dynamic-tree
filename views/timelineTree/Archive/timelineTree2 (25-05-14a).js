...
export class TimelineTree {
    ...
    constructor(selector, person_id) {
        ...
        console.log(`Building new tree for person with ID=${person_id}`);
        ttreePeople   = [];
        ttreeFamilies = [];
...
//===================================================================================
    ...
    static async generateTree (event) {
        ...
        for (var i=0; i<ttreePeople.length; i++) {
            ...
            let xUF = (ttreeFamilies[person["ChildIn"]]["Status"]["Use"] - yearEarliest) * ptsPerYear;
            ...
            for (const famIdx of person["ParentIn"]) {
                var fData = ttreeFamilies[famIdx]["Status"];
                let xPF = (fData["Use"] - yearEarliest) * ptsPerYear;
                let mdot = (fData["Married"]) ? ((fData["Known"]) ? "famDot1" : "famDot3" ) : "famDot3";
            ...
            let xUB = (yearLatest - ttreeFamilies[person["ChildIn"]]["Status"]["Use"]) * ptsPerYear;
            blocksBwd += `<svg id="personBarB-${i}" x="0" y="${barY}">`;
            blocksBwd +=    `<rect x="${xDB}" y="4" width="${xBB-xDB}" height="10" style="fill:${barColour};stroke-width:0;strok    e:#000000"/>`;
            ...
            for (const famIdx of person["ParentIn"]) {
                var fData = ttreeFamilies[famIdx]["Status"];
                let xPF = (yearLatest - fData["Use"]) * ptsPerYear;
                let mdot = (fData["Married"]) ? ((fData["Known"]) ? "famDot1" : "famDot3" ) : "famDot3";
        ...
        for (var i=0; i<ttreePeople.length; i++) {
            ...
            // For Fwd version
            let xF = (ttreeFamilies[person["ChildIn"]]["Status"]["Use"] - yearEarliest) * ptsPerYear;
            ...
            blocksFwd += `</svg>`;
            // For Bwd version
            let xB = (yearLatest - ttreeFamilies[person["ChildIn"]]["Status"]["Use"]) * ptsPerYear;
...
async function ttreeBuildData() {
    ...
    for (var i=0; i<ttreePeople.length; i++) {
        ...
        for (var j=0; j<ttreePeople.length; j++) {
            if (i==j) continue;
...
//        else if (!dl) person["Death"] = {"Year":person["Birth"]["Year"] + 30, "Known":false, "IsLiving":dl}; // Determine this later... (once know marriages)
...
//## FOLLOWING IS NEW #######################################################################################################
    ...
    for (var i=0; i<ttreePeople.length; i++) {
        ...
        if ((person["Father"]["ID"] == 0) && (person["Mother"]["ID"] == 0)) {
            ...
            for (var j=0; j<ttreeFamilies.length; j++) {
                let parents = ttreeFamilies[j]["Parents"];
                if (isParent(parents, person["Father"]["ID"]) && isParent(parents, person["Mother"]["ID"])) { familyIdx = j; break; }
            }
            var family;
            // If no existing family then create a new one
            if (familyIdx == -1) {
                // Create a new family
                family = {"Parents"  : [{"ID": person["Father"]["ID"], "Row": ttreePeople.findIndex(item => item["id"] == person["Father"]["ID"])},
                                        {"ID": person["Mother"]["ID"], "Row": ttreePeople.findIndex(item => item["id"] == person["Mother"]["ID"])}],
                        "Children" : [],
                        "Data"     : {"EarliestBirth": person["Birth"]["Year"], "LatestBirth": person["Birth"]["Year"]},
                        "Status"   : {"Married": false, "Date": null, "Known": false, "Use": null}};
                ttreeFamilies.push(family);
        ...
        var fatherIdx = ttreePeople.findIndex(item => (item["id"]==person["Father"]["ID"]));
        if (fatherIdx >= 0) ttreePeople[fatherIdx]["ParentIn"].add(familyIdx);
        var motherIdx = ttreePeople.findIndex(item => (item["id"]==person["Mother"]["ID"]));
    ...
    for (var i=0; i<ttreePeople.length; i++) {
        ...
        for (var spouseID in person["details"]["Spouses"]) {
            var spouse = ttreePeople.find(item => item["id"] == spouseID);
            ...
            if (familyIdx < 0) {
                var family = {"Parents"  : [{"ID": person["id"], "Row": ttreePeople.findIndex(item => item["id"] == person["id"])},
    ...
    for (var i=0; i<ttreeFamilies.length; i++) {
        var family = ttreeFamilies[i];
    ...
    for (var i=0; i<ttreePeople.length; i++) {
        var person = ttreePeople[i];
        var family = ttreeFamilies[person["ChildIn"]];
...
//## FOLLOWING IS ORIGINAL #######################################################################################################
    ...
    for (var i=0; i<ttreePeople.length; i++) {
        ...
        if (fatherRow >= 0) {
            ...
            else if (person["Birth"]["Year"] < fatherFamily["FirstBirth"]) fatherFamily["FirstBirth"] = person["Birth"]["Year"];
        ...
        if (motherRow >= 0) {
            ...
            else if (person["Birth"]["Year"] < motherFamily["FirstBirth"]) motherFamily["FirstBirth"] = person["Birth"]["Year"];
            // Was the current person born after the last-born in the family?
    ...
    if (typeof motherPerson != 'undefined') {
        ...
        for (const spouseID in mothersSpouses) {
            if (spouseID != fathersID) {
                husbands.push({"id": mothersSpouses[spouseID]["Id"], "details": mothersSpouses[spouseID], "type": "stepParent", "generation": gen, "ParentIn": new Set([])});
    ...
    if (typeof fatherPerson != 'undefined') {
        ...
        for (const spouseID in fathersSpouses) {
            if (spouseID != mothersID) {
                wives.push({"id": fathersSpouses[spouseID]["Id"], "details": fathersSpouses[spouseID], "type": "stepParent", "generation": gen, "ParentIn": new Set([])});
