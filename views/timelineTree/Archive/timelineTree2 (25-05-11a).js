    ...
    async init(container_selector, person_id) {
        ...
        let controlBlock = `
            <div id="controlBlock" class="ttree-not-printable" style="position:sticky; top:1;">
                <label title="The level of data to display">Date to show:&nbsp;</label>
                <select id="paramData" title="Level of data to show">
...
export class TimelineTree {
    ...
    constructor(selector, person_id) {
        ...
        $("#paramData").change(TimelineTree.updateTreeDisplay);
        $("#paramFlip").change(TimelineTree.updateTreeDisplay);
        $("#paramSiblings").change(TimelineTree.updateTreeDisplay);
...
// Class TimelineTree: update display of tree
    ...
    static updateTreeDisplay (event) {
        ...
        var totalHeight = Number(ttreePeople.length * 16);
        elemAll.setAttribute("width", totalWidth);
        elemH.setAttribute("width", totalWidth);
        elemM.setAttribute("height", totalHeight);
...
//===================================================================================
    ...
    static async generateTree (event) {
        ...
        for (var i=0; i<ttreePeople.length; i++) {
            var person = ttreePeople[i];
            rowY = (i * rowHeight) + 14;
            blocksMain += `<svg id="personText-${i} x="0" y="${rowY}">`;
        ...
        for (var i=0; i<ttreePeople.length; i++) {
            ...
            blocksFwd += `<svg id="personBar-${i} x="0" y="${barY}">`;
            ...
            blocksBwd += `<svg id="personBar-${i} x="0" y="${barY}">`;
        ...
        for (var i=0; i<ttreePeople.length; i++) {
            ...
            blocksFwd +=    `<line x1="${xF}" y1="${y1}" x2="${xF}" y2="${y2M}" class="familyLine"/>`;
            blocksFwd +=    `<line x1="${xF}" y1="${y1}" x2="${xF}" y2="${y2F}" class="familyLine"/>`;
            // For Bwd version
            let xB = (yearLatest - person["Family"]["Use"]) * ptsPerYear;
            blocksBwd +=    `<line x1="${xB}" y1="${y1}" x2="${xB}" y2="${y2M}" class="familyLine"/>`;
            blocksBwd +=    `<line x1="${xB}" y1="${y1}" x2="${xB}" y2="${y2F}" class="familyLine"/>`;
        }
        ...
        // And finally, so an initial display   
        TimelineTree.updateTreeDisplay();
