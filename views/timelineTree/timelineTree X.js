...
function updateDisplay (event) {
    ...
    // =============================================================================
    // Determine the ordering of the active people!
    ttreeShow = [];
    ...
    function addToDisplayList(personIdx) {
        ...
        if (motherIdx >= 0) {
            ...
            for (const familyIdx of mother.ParentIn) {
                ...
                if (ttreePeople[spouseRow].Visible && !ttreeShow.includes(spouseRow) && (spouseRow != fatherIdx)) {
                    ttreeShow.push(spouseRow);
                }
            }
        }
        ...
        for (let i=0; i<siblings.length; i++) {
            if (ttreePeople[siblings[i].Row].Visible && !ttreeShow.includes(ttreePeople[siblings[i].Row])) {
                ttreeShow.push(siblings[i].Row);
            }
        }
        ...
        if (fatherIdx >= 0) {
            ...
            for (const familyIdx of father.ParentIn) {
                ...
                if (spouseRow < 0) continue;
                if (ttreePeople[spouseRow].Type != "stepParent") continue;
                if (ttreePeople[spouseRow].Visible && !ttreeShow.includes(spouseRow) && (spouseRow != motherIdx)) {
                    ttreeShow.push(spouseRow);
                }
            }
        }
    ...
    // For each person: show/hide
    for (let i=0; i<ttreePeople.length; i++) {
        const person = ttreePeople[i];
        const show = person.Visible ? 'show' : 'hidden';
        let elem;
        // Show text info
        elem = document.getElementById(`personTextS-${i}`);  elem.setAttribute('visibility', show);
        elem = document.getElementById(`personTextL-${i}`);  elem.setAttribute('visibility', show);
        // Show bars
        elem = document.getElementById(`personSetF-${i}`);   elem.setAttribute('visibility', show);
        elem = document.getElementById(`personSetB-${i}`);   elem.setAttribute('visibility', show);
    }

    // For visible people, adjust location of their details
    for (let i=0; i<ttreeShow.length; i++) {
        const person = ttreePeople[ttreeShow[i]];
        let elem;
        // Show text info
        let y = (i * rowHeight) + 14;
        elem = document.getElementById(`personTextS-${i}`);  elem.setAttribute('y', y);
        elem = document.getElementById(`personTextL-${i}`);  elem.setAttribute('y', y);
        // Show bars
        elem = document.getElementById(`personSetF-${i}`);   elem.setAttribute('y', y);
        elem = document.getElementById(`personSetB-${i}`);   elem.setAttribute('y', y);
    }
    ...
    for (let i=0; i<ttreeFamilies.length; i++) {
        ...
        for (let j = 0; j < family.Parents.length; j++) {
            ...
            const row = ttreeShow.indexOf(family.Parents[j].Row);
        ...
        for (let j = 0; j < family.Children.length; j++) {
            ...
            const row = ttreeShow.indexOf(family.Children[j].Row);
            if ((topRow == null) || (row < topRow)) topRow = row;
            if ((btmRow == null) || (row > btmRow)) btmRow = row;
        }
