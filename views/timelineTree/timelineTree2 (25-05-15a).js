...
window.TimelineTreeView2 = class TimelineTreeView2 extends View {
    ...
    async init(container_selector, person_id) {
        ...
        $("#paramGens").change(refreshDisplay);
        $("#paramData").change(updateDisplay);
        $("#paramFlip").change(updateDisplay);
        $("#paramSiblings").change(updateSiblingsDisplay);
...
function refreshDisplay (event) {
    ...
    const sibs = $("#paramSiblings").prop("checked");
...
function updateSiblingsDisplay (event) {

    const paramSiblings = $("#paramSiblings").prop("checked")
    ...
    for (let i=0; i<ttreePeople.length; i++) {
        ...
        if (sibTypes.includes(person["type"])) {
            ...
            // Turn of any sibling if setting is off
            if (!paramSiblings) {
...
async function generateTree (selector, person_id) {
...
async function ttreeBuildData() {
    ...
    const ancestors_json = await WikiTreeAPI.getAncestors("TimelineTree", ttreePrimaryID, ttreeMaxGens, ancFields);
