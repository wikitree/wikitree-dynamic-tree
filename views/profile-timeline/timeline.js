function getProfileTimeline(data, selector) {
    $(selector).append(`
        <div class="timeline">
    ...
    for (var i = 0; i < Object.keys(x.Parents).length; i++) {
        if (x.Parents[parentIDs[i]].Gender === 'Male') { var relation = 'Father'; }
        else if (x.Parents[parentIDs[i]].Gender === 'Female') { var relation = 'Mother'; }
        else { var relation = 'Parent' }

        if (x.Parents[parentIDs[i]].DeathDate) {
            if (getYearLabel(x.Parents[parentIDs[i]].DeathDate) >= getYearLabel(x.BirthDate) &&
    ...
    for (var i = 0; i < Object.keys(x.Siblings).length; i++) {
        if (x.Siblings[siblingIDs[i]].Gender === 'Male') { var relation = 'Brother'; }
        else if (x.Siblings[siblingIDs[i]].Gender === 'Female') { var relation = 'Sister'; }
        else { var relation = 'Sibling' }

        if (x.Siblings[siblingIDs[i]].BirthDate) {
            if (getYearLabel(x.Siblings[siblingIDs[i]].BirthDate) >= getYearLabel(x.BirthDate) &&
        ...
        if (x.Siblings[siblingIDs[i]].DeathDate) {
            if (getYearLabel(x.Siblings[siblingIDs[i]].DeathDate) >= getYearLabel(x.BirthDate) &&
    ...
    for (var i = 0; i < Object.keys(x.Spouses).length; i++) {
        if (x.Spouses[spouseIDs[i]].Gender === 'Male') { var relation = 'Husband'; }
        else if (x.Spouses[spouseIDs[i]].Gender === 'Female') { var relation = 'Wife'; }
        else { var relation = 'Spouse' }
        if (x.Spouses[spouseIDs[i]].marriage_date) {
            if (getYearLabel(x.Spouses[spouseIDs[i]].marriage_date) >= getYearLabel(x.BirthDate)
        ...
        if (x.Spouses[spouseIDs[i]].marriage_end_date) {
            if (getYearLabel(x.Spouses[spouseIDs[i]].marriage_end_date) >= getYearLabel(x.BirthDate)
                ...
                if (x.Spouses[spouseIDs[i]].marriage_end_date.substring(4, 0) != '0000') {
                    ...
                    timelineEvents.push({
                        ...
                        <div class="timeline__item">
                            <div class="timeline__content">
                                <span class="large">Dissolution of Marriage to ${relation}, ${x.Spouses[spouseIDs[i]].ShortName}</span><br>
                                <p>
        ...
        if (x.Spouses[spouseIDs[i]].DeathDate) {
            if (getYearLabel(x.Spouses[spouseIDs[i]].DeathDate) >= getYearLabel(x.BirthDate)
                ...
                if (x.Spouses[spouseIDs[i]].DeathDate != '0000') {
                    ...
                    timelineEvents.push({
                        ...
                        <div class="timeline__item">
                            <div class="timeline__content">
                                <span class="large">Death of ${relation}, ${x.Spouses[spouseIDs[i]].ShortName}</span><br>
                                <p>
    ...
    for (var i = 0; i < Object.keys(x.Children).length; i++) {
        if (x.Children[childrenIDs[i]].Gender === 'Male') { var relation = 'Son'; }
        else if (x.Children[childrenIDs[i]].Gender === 'Female') { var relation = 'Daughter'; }
        else { var relation = 'Child' }
        if (x.Children[childrenIDs[i]].BirthDate) {
            if (getYearLabel(x.Children[childrenIDs[i]].BirthDate) >= getYearLabel(x.BirthDate) &&
        ...
        if (x.Children[childrenIDs[i]].DeathDate) {
            if (getYearLabel(x.Children[childrenIDs[i]].DeathDate) >= getYearLabel(x.BirthDate) &&
    ...
    function getMother(person) {
        if (person.Mother != null && person.Mother != 0) {
            return `<td style="padding-left:15px;">
                    ...
                    • <a target="_blank" href="https://www.wikitree.com/wiki/${x.Parents[x.Mother].Name}">${x.Parents[x.Mother].Name}</a>
                    </td></tr></table>
            `
        } else {
            return `<td>
    ...
    function getFather(person) {
        if (person.Father != null && person.Father != 0) {
            return `<table style="margin:auto; font-size:initial;"><tr><td style="padding-right:15px;">
                    ...
                    • <a target="_blank" href="https://www.wikitree.com/wiki/${x.Parents[x.Father].Name}">${x.Parents[x.Father].Name}</a>
                    </td>
            `
        } else {
            return `<table style="margin:auto; font-size:initial;"><tr><td style="padding-right:15px;">
    ...
    function getYearLabel(date) {
        ...
        var year = '';
        if (fullDate[0] != "0000") { year = fullDate[0] }
        return year;
    }
    ...
    function prettyDate(date) {
        if (date.includes('-')) {
            ...
            var day = '';
            if (fullDate[0] != "0000") { year = fullDate[0] }
            if (fullDate[1] != "00") { month = `${toMonthName(fullDate[1])} ` }
            if (fullDate[2] != "00") { day = `${fullDate[2]} ` }
