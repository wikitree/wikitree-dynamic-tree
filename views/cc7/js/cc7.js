...
function peopleToTable(kPeople) {
    ...
    kPeople.forEach(function (kPers) {
        ...
        if (oName) {
            ...
            const linkName = htmlEntities(kPers.Name);
            const aLine = $(
                "<tr data-name='" +
                    kPers.Name +
                    "' data-birthdate='" +
...
function timeline(jqClicked) {
    ...
    const familyFacts = [];
    const startDate = getTheYear(tPerson.BirthDate, "Birth", tPerson);
...
function doFamilySheet(fPerson, theClicked) {
    ...
    if (hidIt == false) {
        ...
        let kkTable = peopleToTable(thisFamily);
        ...
        let theLeft = getOffset(theClicked[0]).left;
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                thePeople.forEach(function (aPerson, index) {
                    ...
                    const mSpouses = getRels(mPerson.Spouses, mPerson, "Spouse");
                    mPerson.Spouse = mSpouses;
                    const mChildren = getRels(mPerson.Children, mPerson, "Child");
                    mPerson.Child = mChildren;
                    const mSiblings = getRels(mPerson.Siblings, mPerson, "Sibling");
                    mPerson.Sibling = mSiblings;
                    const mParents = getRels(mPerson.Parents, mPerson, "Parent");
                ...
                doFamilySheet(mPerson, theClicked);
            },
        });
    }
}
...
async function lanceView() {
    ...
    window.people.forEach(function (aPerson) {
        ...
        const linkName = htmlEntities(aPerson.Name);
        const missing = missingThings(aPerson);
        const anLi = $(
            "<li " +
                missing.missingBit +
                " data-first-name='" +
                ...
                familyMembers.forEach(function (aMember) {
                    ...
                    if (thisLI.closest('li[data-name="' + aMember.Name + '"]').length == 0) {
                        ...
                        if (theDegree > aPerson.Degree) {
                            ...
                            const missing = missingThings(aMember);
                            const missingBit = missing.missingBit;
                            const missingIcons = missing.missingIcons;
                            const linkName = htmlEntities(aMember.Name);
                            const bdDates = displayDates(aMember);
                            const anLi = $(
...
function showMissingCounts(jq) {
    ...
    if (thisLI.children("span.countBit,span.nodeCount").length) {
        ...
        const missingFathers =
            thisLI.find("ul img.missingFather").length - thisLI.find("li[data-degree='7'] img.missingFather").length;
        const missingMothers =
            thisLI.find("ul img.missingMother").length - thisLI.find("li[data-degree='7'] img.missingMother").length;
        const missingSpouses =
            thisLI.find("ul img.missingSpouse").length - thisLI.find("li[data-degree='7'] img.missingSpouse").length;
...
async function getPeopleAction(keys, siblings, ancestors, descendants, nuclear, minGeneration, fields) {
    try {
...
function getDegreeAction(event) {
    ...
    if (
        ...
        clearDisplay();
        showShakingTree();
        //$("#peopleTable,#lanceTable,#hierarchyView,.viewButton,#degreesTable").remove();
        $("div.cc7Table").addClass("degreeView");
        const theDegree = $("#cc7Degree").val();
        const fields = "Id";
        getPeopleAction($("#wtid").val(), 0, 0, 0, theDegree, theDegree, fields).then((result) => {
            ...
            while (window.peopleKeys.length) {
                ...
                getSomeRelatives(chunk, oFields).then((result) => {
                    if (result) {
...
function addFamilyToPerson(person) {
    person.Parent = getRels(person.Parents, person, "Parent");
    person.Sibling = getRels(person.Siblings, person, "Sibling");
    person.Spouse = getRels(person.Spouses, person, "Spouse");
    person.Child = getRels(person.Children, person, "Child");
    return person;
}

async function getConnectionsAction(event) {
    wtViewRegistry.clearStatus();
    const theDegree = $("#cc7Degree").val();
    if (
        $("#wtid")
        ...
        const aFields =
            "Parents,Siblings,Spouses,Children,BirthDate,BirthLocation,BirthName,BirthDateDecade,DeathDate,DeathDateDecade,DeathLocation,IsLiving,Father,FirstName,Gender,Id,LastNameAtBirth,LastNameCurrent,Prefix,Suffix,LastNameOther,Derived.LongName,Derived.LongNamePrivate,Manager,MiddleName,Mother,Name,Photo,RealName,ShortName,Touched,DataStatus,Derived.BirthName,Privacy";
        window.ancestors = await getAncestors($("#wtid").val(), 3, aFields);
        assignGeneration(window.ancestors, ancestors[0], 0);
    }
    event.preventDefault();
    ...
    window.people = [];
    clearDisplay();
    getConnections(theDegree);
    showShakingTree();
}

function getConnections(maxDegree, keys = 0) {
    let WTID;
    if (keys == 0) {
        WTID = $("#wtid").val().trim();
    } else {
        ...
        success: function (data) {
            ...
            if (items) {
                items.forEach(function (item) {
                    ...
                    person = addFamilyToPerson(person);
...
function handleFileUpload(event) {
    ...
    reader.onload = async function (e) {
        ...
        window.ancestors = await getAncestors(window.people[0].Id, 3, aFields);
        assignGeneration(window.ancestors, ancestors[0], 0);
...
function cc7excelOut() {
    ...
    $("#cc7csv").click(function () {
        ...
        saveAs(new Blob([s2ab(wbout)], { type: "application/octet-stream" }), makeFilename() + ".csv");
    });
...
async function getAncestors(id, depth, fields = "*") {
    try {
        const result = await $.ajax({
            url: "https://api.wikitree.com/api.php",
            crossDomain: true,
...
function assignGeneration(persons, person, generation) {
    person.Generation = generation;
    for (let ancestor of persons) {
        if (ancestor.Id === person.Father || ancestor.Id === person.Mother) {
            assignGeneration(persons, ancestor, generation + 1);
        }
    }
}
