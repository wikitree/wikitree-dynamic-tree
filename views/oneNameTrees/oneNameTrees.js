...
class LocalStorageManager {
    ...
    proactiveCleanup(newItemSize) {
        ...
        if (currentUsage + newItemSize > maxCapacity) {
            ...
            // Begin regular cleanup process to ensure we stay below the threshold
            while (currentUsage > threshold && this.accessOrder.length > 0) {
                const oldestKey = this.accessOrder.shift();
                const itemSize = localStorage.getItem(oldestKey)?.length || 0;
                localStorage.removeItem(oldestKey);
                currentUsage -= itemSize; // Update current usage to reflect the removed item
                console.log(`Regular cleanup: removed ${oldestKey} to maintain threshold.`);
            }
            this.updateLocalStorageAccessOrder(); // Sync the updated access order after cleanup
        }
    }
