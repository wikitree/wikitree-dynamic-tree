...
async function autofillWTID() {
    if (localStorage.searches) {
        dSearches = localStorage.searches;
        //	console.log(localStorage.searches);
        dSearchSplit = dSearches
            .split("|")
            .sort((a, b) =>
        ...
        fixedSearches = [];
        dSearchSplit.forEach(function (aSearch, index) {
            if (aSearch.match(/\-[0-9].*\-[0-9]/) != null) {
                console.log(aSearch);
                profiles = aSearch.match(/.+?\-[0-9]+/g);
                console.log(profiles);
                profiles.forEach(function (aProfile) {
                    if (!fixedSearches.includes(aProfile)) {
                        fixedSearches.push(aProfile);
                    }
                });
            } else if (!fixedSearches.includes(aSearch) && aSearch.match(/.*?\-[0-9]/) != null) {
                fixedSearches.push(aSearch);
            }
        });
        localStorage.setItem("searches", fixedSearches.join("|") + "|");
        $("#wtid").autocomplete({
            source: fixedSearches,
        });
    }
}
...
$(document).ready(function () {
    if ($("body.ops").length) {
        window.people = [];
        $("#theCategoryGo").click(function () {
            $(".opsStat,#peopleTable,#spLocationFilterLabel,#wideTableButton,#tree,#countdown").remove();
            window.people = [];
            theCat = $("#theCategory").val();
            if (theCat != "") {
                $("h1").text("Category: " + theCat);
                $("title").text("Category: " + theCat);
                getCategoryProfiles(theCat.replaceAll(/ /g, "_"));
            }
        });
        $("#theCategory").on("keydown", function (event) {
            if (event.keyCode === 13) {
                $("#theCategoryGo").click();
            }
        });
    ...
    $("#wtLogo").click(function () {
        window.open("https://www.wikitree.com/");
    });

    login();

    // Autocomplete on WTID box
    if (localStorage.searches) {
        dSearches = localStorage.searches;
        //	console.log(localStorage.searches);
        dSearchSplit = dSearches
...
function sortList(aList, dataField, reverse = 0) {
    rows = aList.find("li");
    rows.sort(function (a, b) {
        if (reverse == 0) {
    ...
    rows.appendTo(aList);
    secondarySort3(aList, "lnab", "first-name", 1);
}

async function lanceView() {
    $("#peopleTable").hide();
    if (!window.surnames) {
        window.surnames = {
            degree_1: [],
    ...
    window.people.forEach(function (aPerson) {
        if (!aPerson.Missing) {
            aPerson = missingBits(aPerson);
        }
        let theDegree = aPerson.Degree;
        let wanted = [
            "Prefix",
            "FirstName",
            "MiddleNames",
        ...
        let aName = new PersonName(aPerson);
        let theName = aName.withParts(wanted);
        let theLNAB = aName.withParts(["LastNameAtBirth"]);
        let theFirstName = aName.withParts(["FirstName"]);
        ...
        if (isOK(theDegree)) {
            if (!window.surnames["degree_" + theDegree].includes(theLNAB)) {
                ...
                anLi2.find("a").on("click", function (e) {
                    ...
                    let degreeNum = $(this).closest("th").prop("id").slice(-1);
                    let theList = $("#degree_" + degreeNum).find("li");
                    if ($(this).attr("data-clicked") == 1) {
                        ...
                        thisLNAB = $(this).closest("li").data("lnab");
                        $("#lanceTable thead a").each(function () {
                            $(this).attr("data-clicked", 0);
                        });
        ...
        let linkName = htmlEntities(aPerson.Name);
        let missing = missingThings(aPerson);
        let missingBit = missing.missingBit;
        let missingIcons = missing.missingIcons;
        let anLi = $(
            "<li " +
                missingBit +
                " data-first-name='" +
...
function missingBits(aPerson) {
    ...
    if (
        (ageAtDeath(aPerson, false) > 15 || aPerson?.DeathDate == "0000-00-00" || aPerson?.BirthDate == "0000-00-00") &&
...
function missingThings(aPerson) {
    let missingBit = "";
    let missingIcons = "";
    aPerson.Missing.forEach(function (relation) {
        ...
        if (relation == "Father") {
            missingIcons +=
                "<img title='missing father' class='missingFather missingIcon' src='images/blue_bricks_small.jpg'>";
        }
        if (relation == "Mother") {
            missingIcons +=
                "<img  title='missing mother' class='missingMother missingIcon' src='images/pink_bricks_small.jpg'>";
        }
        if (relation == "Spouse") {
            missingIcons +=
                "<img  title='possible missing spouse' class='missingSpouse missingIcon' src='images/spouse_bricks_small.png'>";
        }
        if (relation == "Children") {
            missingIcons +=
                "<img   title='possible missing children' class='missingChildren missingIcon' src='images/baby_bricks_small.png'>";
        }
    });
    return { missingBit: missingBit, missingIcons: missingIcons };
}

function addPeopleToHierarchy(degree) {
    $("#hierarchyView li[data-degree='" + degree + "']").each(function () {
        let wtid = $(this).data("name");
        let thisLI = $(this);
        window.people.forEach(function (aPerson) {
            if (aPerson.Name == wtid) {
                let familyMembersKeys = [];
                if (aPerson.Parents) {
                    if (aPerson.Father) {
                ...
                let relations = ["Siblings", "Spouses", "Children"];
                relations.forEach(function (aRelation) {
                    if (aPerson[aRelation]) {
                        let relKeys = Object.keys(aPerson[aRelation]);
                ...
                familyMembersKeys.forEach(function (aKeyPair) {
                    window.people.forEach(function (oPerson) {
                        if (aKeyPair.id == oPerson.Id) {
                            oPersonClone = JSON.parse(JSON.stringify(oPerson));
                ...
                familyMembers.forEach(function (aMember) {
                    aMember = missingBits(aMember);

                    if (thisLI.closest('li[data-name="' + aMember.Name + '"]').length == 0) {
                        let theDegree = aMember.Degree;
                        if (theDegree > aPerson.Degree) {
                            ...
                            let aName = new PersonName(aMember);
                            let theName = aName.withParts(wanted);
                            let theLNAB = aName.withParts(["LastNameAtBirth"]);
                            let theFirstName = aName.withParts(["FirstName"]);
                            let relation;
                            relation = aMember.Relation;
                            ...
                            let missing = missingThings(aMember);
                            let missingBit = missing.missingBit;
                            let missingIcons = missing.missingIcons;
                            let linkName = htmlEntities(aMember.Name);
                            let bdDates = displayDates(aMember);
                            let anLi = $(
                                `<li data-birth-date='${aMember.BirthDate}' data-father='${aMember.Father}' data-mother='${aMember.Mother}' data-id='${aMember.Id}' data-relation='${relation}' ${missingBit} data-lnab='${theLNAB}' data-degree='${aMember.Degree}' data-name=\"${aMember.Name}\" data-first-name='${theFirstName}'>${aMember.Degree}° <span class='relation ${relation}'>${relation}</span>: <a target='_blank' href='https://www.wikitree.com/wiki/${linkName}'>${theName}</a> <span class='birthDeathDates'>${bdDates}</span> ${missingIcons}<ul></ul></li>`
                            );
...
function hierarchyCC7() {
    ...
    const hierarchySection = $(
        "<section id='hierarchyView'><menu><button class='button small' id='showAllDegrees'>Expand All</button>" +
            "<button id='showOneMoreDegree' class='button small'>+</button><button class='button small' id='showOneFewerDegree'>−</button></menu><ul></ul></section>"
    );
    hierarchySection.insertBefore($("#peopleTable"));
    let aPerson = window.people[0];
    let wanted = [
        "Prefix",
        "FirstName",
        "MiddleNames",
    ...
    let linkName = htmlEntities(aPerson.Name);
    let anLi = $(
        `<li data-lnab='${theLNAB}' data-id='${aPerson.Id}' data-degree='${aPerson.Degree}' data-name=\"${aPerson.Name}\" data-first-name='${theFirstName}'>${aPerson.Degree}° <a target='_blank' href='https://www.wikitree.com/wiki/${linkName}'>${theName}</a><ul></ul></li>`
    );
    hierarchySection.children("ul").append(anLi);
    for (let i = 0; i < 7; i++) {
        addPeopleToHierarchy(i);
    }
    $("#hierarchyView li").each(function () {
        ...
        if ($(this).find("ul li").length) {
            ...
            showMissingCounts(aButton);
            aButton.on("click", function (e) {
                e.preventDefault();
                $(this).parent().children("ul").toggle();
                $(this).text($(this).text() == "+" ? "−" : "+");
                showMissingCounts($(this));
            });
        } else {
