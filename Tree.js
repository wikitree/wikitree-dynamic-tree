$(document).ready(function() {
  // Before we can show the user the Tree Viewer, we want them to be logged into the WikiTree API.
  ...
  // If the user is not yet logged into the API, show them the form that sends them there to login, after which they'll return here.
  ...
  var userName = $.cookie('WikiTreeAPI_userName');
  var userId = $.cookie('WikiTreeAPI_userId');
  var u = new URLSearchParams(window.location.search)
  var authcode = u.get('authcode');

  if ((typeof(userName) != 'undefined') && (userName != null) && (userName != '')) {
    // We have a user that logged into the API. Show them the Tree Viewer content.
    $('#treeViewerContent').show().removeClass('inactive');
    $('#treeViewerCanvasContainer').show().removeClass('inactive');
    viewTreePersonId = userId;
    var tree = new TreeViewer('#treeViewerCanvasContainer', viewTreePersonId);
    WikiTreeAPI.postToAPI( {'action':'getPerson', 'key':viewTreePersonId } )
    .then(function(data) {
        updateViewedPersonContent(data[0].person);
    });
  }
  else if ((typeof authcode != 'undefined') && (authcode != null) && (authcode != '')) {
    // We have an auth code to confirm.
    $('#confirmAuth').show();
    WikiTreeAPI.postToAPI( { 'action':'clientLogin', 'authcode': authcode } )
    .then(function(data) {
      if (data.clientLogin.result == 'Success') {
          $.cookie('WikiTreeAPI_userName', data.clientLogin.username, { 'path': '/'} );
          $.cookie('WikiTreeAPI_userId', data.clientLogin.userid, { 'path': '/'} );
    ...
    // The user has to click the button in the form to go login at the API.
    $('#getAPILogin').show();
  }
...
/*
 * The Tree Viewer itself uses the d3.js library to render the graph.
 */
...
(function(){
  ...
  Tree.prototype.personPopup = function(person, event){
    ...
    d3.select('#treeViewerCanvasContainer').on('click', function(){
      popup.remove();
    });
  };
...
function updateViewedPersonContent(person) {
  $('#treeNavigation').html(`
      ...
      <div id="dropdown-items" class="dropdown-content">
        <a href="https://apps.wikitree.com/apps/clarke11007/fan.php?ph=1&id=${person.Id}&NumGens=5">Fan Chart</a>
      </div>
    `);

  // hide the dropdown when clicking away
  $('html').on('click', function(e){
  	if(e.target.id !== 'dropdown-button' && $(e.target).parents('#dropdown-button').length === 0) {
  ...
  $('.viewTreePersonBirthName').html(person.BirthName);
