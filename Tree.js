...
function launchTree(viewTreeId, viewTreePersonId, viewTreePersonName) {
	...
	if (viewTreeId == 'profile-timeline') {
		$('#treeInfo').load(
			'views/profile-timeline/treeInfo.html',
			function () {
				WikiTreeAPI.postToAPI({ 'action': 'getPerson', 'key': viewTreePersonId, 'fields': 'Categories,Father,Mother,Derived.ShortName,Name,BirthDate,DeathDate,TrustedList,BirthLocation,DeathLocation,Parents,Children,Spouses,Siblings,Gender,Photo,PhotoData,Privacy,DataStatus,Bio' })
					.then(function (data) {
						updateViewedPersonContent(data[0].person);
						getProfileTimeline(data[0].person, '#treeViewerContainer');
					});
			}
		);
	}
	// Location Timeline
	if (viewTreeId == 'location-timeline') {
		$('#treeInfo').load(
			'views/location-timeline/treeInfo.html',
			function () {
				WikiTreeAPI.postToAPI({ 'action': 'getPerson', 'key': viewTreePersonId, 'fields': 'Derived.ShortName,BirthDate,DeathDate,BirthLocation,DeathLocation,Children,Spouses,Parents,Bio' })
					.then(function (data) {
						updateViewedPersonContent(data[0].person);
						getLocationTimeline(data[0].person, 'treeViewerContainer');
					});
			}
		);
	}
	// Chronology chart
	if (viewTreeId == 'chronology-chart') {
		$('#treeInfo').load(
