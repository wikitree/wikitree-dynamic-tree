...
function launchTree(viewTreeId, viewTreePersonId, viewTreePersonName) {
    ...
    if (viewTreeId == "wikitree-dynamic-tree") {
        $("#treeInfo").load("views/baseDynamicTree/treeInfo.html", function () {
            WikiTreeAPI.postToAPI({
                action: "getPerson",
                key: viewTreePersonId,
                fields: infoFields,
            }).then(function (data) {
    ...
    if (viewTreeId == "restyled-base") {
        $("#treeInfo").load("views/restyledBaseExample/treeInfo.html", function () {
            WikiTreeAPI.postToAPI({
                action: "getPerson",
                key: viewTreePersonId,
                fields: infoFields,
...
function newTree(k) {
    var key;
    if (k) {
        $("#viewTreePersonName").val(k);
    }
    var key = $("#viewTreePersonName").val();

    WikiTreeAPI.postToAPI({ action: "getPerson", key: key }).then(function (data) {
        if (data.error) {
